...
// Fixed-config reference agent: puct-ppat-fp (puct-ppat with featurepol as
// the prior/pruning policy in place of npat) with exactly 1000 playouts per
// move — self-contained copy, frozen 2026-08-20 as a ladder rung; frozen
// names must never track live files.  All parameters are hardcoded; this
// script reads no environment variables.
//
...
function create() {
  ...
  const fpModel     = FeaturePol.loadModel({ name: 'ref-puct-ppat-fp-1k',
    path: _isBrowser ? undefined
                     : require('path').join(__dirname, '..', 'ref', 'featurepol-6082.js') });
  const fpWeights   = fpModel.weights;
  console.log(`ref-puct-ppat-fp-1k: ${_model ? _model.weights.length : 0} ppat weights, ${fpWeights.size} featurepol weights from ${fpModel.modelName}`);
  ...
  function makeNode(move, parent, ci, game2, N, game3) {
    ...
    if (TOP_K > 0 && parent !== null) {
      movesArr = _pruneToTopK(movesArr, fpState, TOP_K, N);
    }
