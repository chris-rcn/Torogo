...
// Softmax-sampling wrapper for the argmax-distilled npat policy.
//
// Weights from out/npat-distill-3k.js (npat distilled from vlibpat-ref-3x3
// via argmax cross-entropy: 3k training games at LR=0.05, epsilon=0.1).
//
// Used to compare apples-to-apples (softmax vs softmax) against
// ref-npat-softmax — counterpart to ai/npat-distill-softmax.js (which uses
// the value-based distillation weights).
...
const WEIGHTS_PATH = path.join(__dirname, '..', 'ref', 'npat-distill-3k.js');

const raw = require(path.resolve(WEIGHTS_PATH));
if (raw.tactStoneLimit !== undefined && raw.tactStoneLimit !== NPat.TACT_STONE_LIMIT) {
  throw new Error(
    `npat-distill-argmax-softmax: TACT_STONE_LIMIT mismatch — file ${path.basename(WEIGHTS_PATH)} ` +
...
console.error(`npat-distill-argmax-softmax: loaded ${weights.size} weights from ${path.basename(WEIGHTS_PATH)} ` +
