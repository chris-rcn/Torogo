...
// Fixed-config reference agent: PUCT-style MCTS with RAVE blend for Q,
// npat-driven priors, and top-K candidate pruning at every node.
//
// Differs from prod.js only in node selection: this variant uses
//   score = Q + C_PUCT · P(s,a) · √N_total / (1 + N_a)
// where Q is the same RAVE/real blended win-rate as prod.js, and P(s,a) is
// the npat softmax probability for the move (PASS and out-of-extract moves
// get a small uniform floor, then priors are renormalised to sum to 1).
//
...
// Note: prod.js's npat-seeded RAVE pathway is intentionally removed here.
// PUCT already biases early exploration toward high-npat moves; seeding RAVE
// from the same prior would double-count it.  RAVE itself is kept because it
// captures playout-derived signal that npat can't supply.
...
const C_PUCT        = Util.envFloat('C_PUCT', 1.5);
const RAVE_K        = 1200;
const N_EXPAND      = 3;
...
function nodeScore(moveIdx, node, rng) {
  ...
  // Q: RAVE/real blended win-rate (unchanged from prod.js).
  const Q = realWeight * realWR + raveWeight * raveWR;

  // PUCT exploration term.  C_PUCT · P(s,a) · √N_total / (1 + N_a).
  const P = node.priors[moveIdx];
  const U = C_PUCT * P * Math.sqrt(node.totalVisits) / (1 + realV);
