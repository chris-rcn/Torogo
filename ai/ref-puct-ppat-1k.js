...
// Fixed-config reference agent: puct-ppat with exactly 1000 playouts per
...
function create() {
  ...
  const npatModel   = NPat.loadModel({ name: 'ref-puct-ppat-1k' });
  const npatWeights = npatModel.weights;
  console.log(`ref-puct-ppat-1k: ${_model ? _model.weights.length : 0} ppat weights, ${npatWeights.size} npat weights from ${npatModel.modelName}`);
