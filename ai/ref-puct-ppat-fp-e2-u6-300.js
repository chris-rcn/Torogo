...
// ppat playouts) with exactly 300 playouts per move, uniform playout moves
// below board fullness 0.6 (PPAT_UNIFORM_BELOW_PHASE), and N_EXPAND=2 lazy
...
// Identical to ref-puct-ppat-fp-e2-u6-1k except for the playout count, which
// places a rung between ref-puct-ppat-300 and the 1k rung.
// Self-contained copy, frozen 2026-08-26 as a ladder rung; frozen names
...
function create() {
  ...
  const fpModel     = FeaturePol.loadModel({ name: 'ref-puct-ppat-fp-e2-u6-300',
    path: _isBrowser ? undefined
                     : require('path').join(__dirname, '..', 'ref', 'featurepol-6082.js') });
  const fpWeights   = fpModel.weights;
  console.log(`ref-puct-ppat-fp-e2-u6-300: ${_model ? _model.weights.length : 0} ppat weights, ${fpWeights.size} featurepol weights from ${fpModel.modelName}`);
  ...
  // Runs a full search (PLAYOUTS playouts, default 300) and returns the root win
