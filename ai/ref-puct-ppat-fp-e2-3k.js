...
// ppat playouts) with exactly 3000 playouts per move and N_EXPAND=2 lazy
...
function create() {
  ...
  const fpModel     = FeaturePol.loadModel({ name: 'ref-puct-ppat-fp-e2-3k',
    path: _isBrowser ? undefined
                     : require('path').join(__dirname, '..', 'ref', 'featurepol-6082.js') });
  const fpWeights   = fpModel.weights;
  console.log(`ref-puct-ppat-fp-e2-3k: ${_model ? _model.weights.length : 0} ppat weights, ${fpWeights.size} featurepol weights from ${fpModel.modelName}`);
