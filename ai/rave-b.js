...
//
// Identical to ai/rave.js EXCEPT it reads its constants from *_B env vars
// (RAVE_K_B, EXPLORATION_C_B, N_EXPAND_B, RAVE_INHERIT_B, PLAYOUTS_B).  This
// exists purely as an unbiased head-to-head tuning anchor: run
// `--p1 rave --p2 rave-b` and the two players take independent constants from
// one process (the shared-module-cache issue that blocks `rave` vs `rave`).
// Keep this engine bit-identical to rave.js — only the env names differ.
