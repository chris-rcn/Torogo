...
// Normalise a raw vlibpat data file's weight table to { count, forEach(cb) },
...
//     — produced by saveWeights; weight = qvals[i] / scale.
...
// ({ specs, weights|keys+vals, tactics?, ... } — e.g. a required model file or
...
// Weights are stored base64-packed as Int32 keys followed by int16-quantised
// vals (a per-file symmetric scale maps [-maxAbs, maxAbs] onto int16):
// weight ≈ qval / scale.  Far smaller than a literal Map, and the int16 step
// (~maxAbs/32767) is negligible against logits summed over many features.
// Loaded by modelWeights (which also still reads legacy Map files).
...
function saveWeights(filePath, model) {
  ...
  const count = model.weights.size;
  let maxAbs = 0;
  for (const [, v] of model.weights) { const a = v < 0 ? -v : v; if (a > maxAbs) maxAbs = a; }
  const scale = maxAbs > 0 ? 32767 / maxAbs : 1;

  const keys  = new Int32Array(count);
  const qvals = new Int16Array(count);
  let i = 0;
  for (const [k, v] of model.weights) {
    keys[i] = k;
  ...
  const src = [
    ...
    `  return { specs${extras}, count, scale, keys, qvals };`,
    "})();",
