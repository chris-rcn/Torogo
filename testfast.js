...

{
  ...
  // Helper: compare getLadderStatus2 and getLadderStatus3 on every stone with
  // 1–2 liberties in a position.  Returns null on success, error string on failure.
  function compareLadderStatus(g2, g3) {
    const cap = g2.N * g2.N;
    const visited = new Uint8Array(g2._nextGid + 1);
    for (let i = 0; i < cap; i++) {
      ...
      const r2 = getLadderStatus2(g2, i);
      const r3 = getLadderStatus3(g3, i);

      if (r2 === null && r3 === null) continue;
      if (r2 === null || r3 === null) return `stone ${i}: one null, other not`;
      if (r2.length !== r3.length)    return `stone ${i}: length mismatch ${r2.length} vs ${r3.length}`;

      for (let k = 0; k < r2.length; k++) {
        const a = r2[k], b = r3[k];
        if (a.liberty.x !== b.liberty.x || a.liberty.y !== b.liberty.y)
          return `stone ${i} entry ${k}: liberty mismatch`;
        if (a.canEscape !== b.canEscape)
          return `stone ${i} entry ${k}: canEscape ${a.canEscape} vs ${b.canEscape}`;
        if (a.canEscapeAfterPass !== b.canEscapeAfterPass)
          return `stone ${i} entry ${k}: canEscapeAfterPass ${a.canEscapeAfterPass} vs ${b.canEscapeAfterPass}`;
      }
    }
    return null;
  }

  // Play the same move sequence on both a Game2 and a Game3 simultaneously.
  function makePair(N) {
    return { g2: new Game2(N), g3: new Game3(N) };
  }
  function playBoth({ g2, g3 }, idx) {
    g2.play(idx);
    g3.play(idx);
  }

  section('ladder3 vs ladder2: empty board');
  {
    const { g2, g3 } = makePair(9);
    assert(compareLadderStatus(g2, g3) === null, 'empty board: no groups to check');
  }

  section('ladder3 vs ladder2: simple 1-lib group (atari)');
  {
    // Build a stone in atari: 9x9, place black at (4,4), surround 3 sides with white.
    //   . W .
    //   W B W   ← black at (4,4) has 1 liberty above
    //   . . .
    const { g2, g3 } = makePair(9);
    const N = 9;
    const idx = (x, y) => y * N + x;
    for (const [x, y] of [[4,4],[5,4],[3,4],[4,5]]) {
      const color = (x === 4 && y === 4) ? BLACK : WHITE;
      g2.current = color; g3.current = color;
      playBoth({ g2, g3 }, idx(x, y));
    }
    g2.current = BLACK; g3.current = BLACK;
    const err = compareLadderStatus(g2, g3);
    assert(err === null, `simple atari: ${err}`);
  }

  section('ladder3 vs ladder2: classic ladder');
  {
    // Classic 9x9 ladder: black stone at (1,1), white closes in from right & top.
    const { g2, g3 } = makePair(9);
    const N = 9;
    const idx = (x, y) => y * N + x;
    // Black stone to chase
    g2.current = BLACK; g3.current = BLACK;
    playBoth({ g2, g3 }, idx(1,1));
    // White attackers surround right and bottom
    for (const [x, y] of [[2,1],[1,2]]) {
      g2.current = WHITE; g3.current = WHITE;
      playBoth({ g2, g3 }, idx(x, y));
    }
    g2.current = BLACK; g3.current = BLACK;
    const err = compareLadderStatus(g2, g3);
    assert(err === null, `classic ladder: ${err}`);
  }

  section('ladder3 vs ladder2: 2-liberty group');
  {
    const { g2, g3 } = makePair(9);
    const N = 9;
    const idx = (x, y) => y * N + x;
    // Black chain at (4,4)-(4,5), white on three sides leaving two libs
    const moves = [
      [BLACK, 4,4], [WHITE, 5,4], [BLACK, 4,5], [WHITE, 5,5],
      [WHITE, 3,4], [WHITE, 3,5], [WHITE, 4,6],
    ];
    for (const [color, x, y] of moves) {
      g2.current = color; g3.current = color;
      playBoth({ g2, g3 }, idx(x, y));
    }
    g2.current = BLACK; g3.current = BLACK;
    const err = compareLadderStatus(g2, g3);
    assert(err === null, `2-liberty group: ${err}`);
  }

  section('ladder3 vs ladder2: game3 state unchanged after call');
  {
    const { g2, g3 } = makePair(9);
    const N = 9;
    const idx = (x, y) => y * N + x;
    g2.current = BLACK; g3.current = BLACK;
    playBoth({ g2, g3 }, idx(4,4));
    g2.current = WHITE; g3.current = WHITE;
    playBoth({ g2, g3 }, idx(5,4));
    g2.current = WHITE; g3.current = WHITE;
    playBoth({ g2, g3 }, idx(4,5));
    g2.current = WHITE; g3.current = WHITE;
    playBoth({ g2, g3 }, idx(3,4));
    g2.current = BLACK; g3.current = BLACK;

    // Capture snapshot of g3 state before call.
    const cellsBefore = new Int8Array(g3.cells);
    const currentBefore = g3.current;
    const stackLenBefore = g3._undoStack.length;

    getLadderStatus3(g3, idx(4,4));

    assert(g3.current === currentBefore, 'state unchanged: current');
    assert(g3._undoStack.length === stackLenBefore, 'state unchanged: undo stack length');
    let cellsOk = true;
    for (let i = 0; i < N * N; i++) {
      if (g3.cells[i] !== cellsBefore[i]) { cellsOk = false; break; }
    }
    assert(cellsOk, 'state unchanged: cells');
  }

  section('ladder3 vs ladder2: random positions stress test');
  {
    const TRIALS = 8;
    const MOVES_PER_TRIAL = 30;
    let ok = true;
    let prng = 777;
    const rand = () => { prng = Math.imul(prng, 1664525) + 1013904223 >>> 0; return prng; };
    ...
    for (let t = 0; t < TRIALS && ok; t++) {
      ...
      for (let m = 0; m < MOVES_PER_TRIAL; m++) {
        const idx = rand() % cap;
        if (g2.cells[idx] !== 0 || !g2.isLegal(idx)) continue;
        playBoth({ g2, g3 }, idx);

        const err = compareLadderStatus(g2, g3);
        if (err !== null) { ok = false; break; }
      }
    }
