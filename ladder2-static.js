...
// Returns:  0 = no captures detected
//          -1 = capture detected (fall back — virtual stone model can't
//               accurately track post-capture liberties)
function _handleCaptures(board, nbr, cap, lib, atkColor) {
  ...
  for (let d = 0; d < 4; d++) {
    ...
    if (!_atkGroupHasOtherLib(board, nbr, cap, ni, lib, atkColor)) {
      return -1;  // capture detected — fall back
    }
  }
  return 0;
}
...
function _libConnects(nbr, cells, gidArr, ls, lib, defColor) {
  ...
  for (let d = 0; d < 4; d++) {
    const ni = nbr[base + d];
    if (cells[ni] === defColor && !_groupMask[ni] && ls[gidArr[ni]] >= 5) return true;
  }
  return false;
}
...
function _traceRec(game2, board, nbr, cap, idx, defColor, atkColor, current, depth, ko) {
  ...
  if (current === defColor) {
    ...
    for (let li = 0; li < lc; li++) {
      ...
      if (_traceLog && capResult !== 0) console.log(`${'  '.repeat(depth)}  capture → fallback`);

      const result = capResult === -1 ? null :
  ...
  for (let li = 0; li < lc; li++) {
    ...
    if (afterLc === 0) {
      ...
      if (_traceLog) console.log(`${'  '.repeat(depth)}ATK block ${cs(lib)} afterLc=${afterLc}`);
