...
// tune-ravepat.js — optimize pattern1.js weights using move-quality evaluation.
//
// CMA-ES minimizes the rmsErr of ravepat move choices against pre-computed
// KataGo win-rate rankings (produced by createmovedetails.js).
//
// Usage:
//   node tune-ravepat.js --file <details> [--budget <ms>] [options]
//
...
const displayArg  = parseInt(get('--display-positions', '100'),   10);
...
const discoveryN  = parseInt(get('--discovery',      '100'), 10);
const outFile     = get('--out', `tuned-weights-${Math.floor(Math.random() * 1000000)}.js`);

if (!file || budgetMs <= 0) {
  process.stderr.write('Usage: node tune-ravepat.js --file <details> --budget <ms> [options]\n');
  process.exit(1);
}
...
// ── Pattern discovery ─────────────────────────────────────────────────────────
// Play random games and collect every pattern1 hash for each legal non-eye move.
  ...
  while (!game2.gameOver) {
    ...
    if (candidates.length > 0) {
      ...
      for (const { hash } of hashes) {
        if (!patternIndex.has(hash)) patternIndex.set(hash, patternIndex.size);
      }
    }
...
const patternKeys = new Array(n);
for (const [h, i] of patternIndex) patternKeys[i] = h;
...
function makeAgent(weights) {
  const table = new Map();
  for (let i = 0; i < n; i++) table.set(patternKeys[i], [sigmoid(weights[i]), 1e6]);
...
function saveWeights() {
  const body = patternKeys.map((h, i) => `[${h},[${sigmoid(cma.m[i])},1000000]]`).join(',');
  fs.writeFileSync(outFile,
    `'use strict';\n` +
    `// Auto-generated by tune-ravepat.js — do not edit by hand.\n` +
