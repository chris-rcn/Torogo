...
class Game2 {
  constructor(size, applyFirstMove = true) {
    ...
    this._gid  = new Int16Array(cap).fill(-1);
    this._nextGid = 0;
    this._freeGids = new Int16Array(cap);  // group reuse stack
    this._nFreeGids = 0;

    // Bitset width: words per group bitset (ceil(cap/32))
    const W = (cap + 31) >> 5;
    this._W = W;
    const MAX_G = cap + 4;  // reduced from 4*cap+4 with group reuse
    this._gc = new Uint8Array(MAX_G);       // group color
    this._sw = new Int32Array(MAX_G * W);   // stones bitset words (must stay 32-bit for bitops)
    ...
    this._lastCaptures      = new Int16Array(cap);
    this._lastCaptureCount  = 0;
    this._lastCaptureChains = 0;

    this._emptyCells = new Int16Array(cap);
    this._emptySlot  = new Int16Array(cap);
  ...
  _allocGid() {
    if (this._nFreeGids > 0) return this._freeGids[--this._nFreeGids];
    return this._nextGid++;
  }

  _freeGid(gid) {
    this._freeGids[this._nFreeGids++] = gid;
  }
  ...
  _place(idx, color) {
    ...
    if (ns === 0) {
      const gid = this._allocGid();
      gidArr[idx] = gid;
      this._gc[gid] = color;
      // Clear bitsets for reused slot
      const gb0 = gid * W;
      for (let wi = 0; wi < W; wi++) { sw[gb0 + wi] = 0; lw[gb0 + wi] = 0; }
      sw[gb0 + (idx >> 5)] |= (1 << (idx & 31));
      ss[gid] = 1;
      const lb = gid * W;
      let lsCount = 0, m, wi;
  ...
  // Skip legality check — caller guarantees the move is legal.
  playUnchecked(idx) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (ls[gid] === 0) {
        ...
        if (gsize === 1) capturedIdx = lastCap[nCapBefore];
        nChains++;
        this._freeGid(gid);
      }
    }
  ...
  toString(markIdx = this.lastMove, { centerAt = null, labels = false } = {}) {
    ...
    const rows = [];
    if (labels) {
      let header = ' '.repeat(labelW + 1);
      for (let dx = 0; dx < N; dx++) {
        if (dx > 0) header += ' ';
        header += String.fromCharCode(97 + (x0 + dx) % N);
      }
      rows.push(header);
    }
  ...
  clone() {
    const cap = this.N * this.N;
    const totalG = this._gc.length;
    const totalBits = this._sw.length;
    const g = Object.create(Game2.prototype);
    g.N = this.N; g.boardSize = this.N;
    g._W = this._W;
    g._nbr = this._nbr; g._dnbr = this._dnbr;
    g._allCells = this._allCells; g.nbr = this._nbr;
    g._lastCaptures = this._lastCaptures;
    g.cells = new Int8Array(this.cells);
    g._gid = new Int16Array(this._gid);
    g._freeGids = new Int16Array(this._freeGids.length);
    g._gc = new Uint8Array(totalG);
    g._ss = new Int16Array(totalG);
    ...
    // Partial copy of group arrays
    const ng = this._nextGid;
    const ub = ng * this._W;
    g._freeGids.set(this._freeGids.subarray(0, this._nFreeGids));
    g._gc.set(this._gc.subarray(0, ng));
    g._ss.set(this._ss.subarray(0, ng));
    g._ls.set(this._ls.subarray(0, ng));
    g._sw.set(this._sw.subarray(0, ub));
    g._lw.set(this._lw.subarray(0, ub));
    g._nextGid = this._nextGid;
    g._nFreeGids = this._nFreeGids;
    g.current = this.current;
    g.ko = this.ko;
    g.koStone[0] = this.koStone[0]; g.koStone[1] = this.koStone[1]; g.koStone[2] = this.koStone[2];
  ...
  // Copy state into a pre-allocated target (same shape as this).
  cloneInto(g) {
    g.cells.set(this.cells);
    g._gid.set(this._gid);
    g._nextGid = this._nextGid;
    g._nFreeGids = this._nFreeGids;
    g._freeGids.set(this._freeGids.subarray(0, this._nFreeGids));
    const ng = this._nextGid;
    const usedBits = ng * this._W;
    g._gc.set(this._gc.subarray(0, ng));
    g._ss.set(this._ss.subarray(0, ng));
    g._ls.set(this._ls.subarray(0, ng));
    g._sw.set(this._sw.subarray(0, usedBits));
    g._lw.set(this._lw.subarray(0, usedBits));
    g.current = this.current;
    g.ko = this.ko;
    g.koStone[0] = this.koStone[0]; g.koStone[1] = this.koStone[1]; g.koStone[2] = this.koStone[2];
    g.consecutivePasses = this.consecutivePasses;
    ...
    g._lastCaptureCount = this._lastCaptureCount;
    g._lastCaptureChains = this._lastCaptureChains;
    g._emptyCells.set(this._emptyCells);  // could partial-copy but emptySlot needs full copy
    g._emptySlot.set(this._emptySlot);
    return g;
  }
